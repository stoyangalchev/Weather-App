...
export default function CurrentDayCard() {
  ...
  return (
    ...
    >
      <div className='relative border-[0px] rounded-2xl bg-gray-900 bg-opacity-20   border-slate-900 col-span-2 h-fit w-full grid grid-cols-2 sm:grid-cols-4 grid-rows-4 justify-center items-center px-5 sm:px-10'>
        <div className='col-start-1 row-start-1'>
          <h3 className='mt-10 text-3xl place-self-start text-gray-100'>
            {DAY[new Date(current.last_updated).getDay()] || 'Today'}
          </h3>
          <h3 className='mb-6 text-md sm:text-xl text-gray-200 text-gray-200'>
        ...
        <div className='flex flex-col col-start-4 mt-4 place-items-end'>
          <div className='flex items-center flex-col-reverse sm:flex-row'>
            <h2 className='col-span-2 text-2xl md:text-4xl font-bold sm:text-5xl text-gray-100'>
              {degreeType === 'C'
                ? current.temp_c + 'ºC'
                : current.temp_f + 'ºF'}
            </h2>
      ...
      <Map
        isMarkerShown
        className=' rounded-2xl items-end w-full row-span-1 h-96 md:h-full'
      />
    </div>
  )
}
